...
def _local_hermetic(monkeypatch, tmp_path):
    ...
    # default_stt_engine lives on system_config_store's `engines` group, not
    # Settings. Patch the shared singleton's .get() (not .set()) so this never
    # writes through to the shared config_system DB row.
    _real_get = system_config_store.get

    def _get_with_stub_stt():
        cfg = _real_get()
        return cfg.model_copy(update={
            "engines": cfg.engines.model_copy(update={"default_stt_engine": "stub-mcp-stt"})
        })
    ...
    # "stub-mcp-stt" isn't recognized by stt_service's real engine-listing
    ...
    # check_resolved_engines() would refuse/KeyError before the device-tool
