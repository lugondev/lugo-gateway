...
def test_status_error_names_the_provider_status_and_body():
    ...
    err = translate_httpx_error("http_stt", exc)
    assert isinstance(err, RuntimeError)
    assert str(err) == "http_stt returned HTTP 401: invalid bearer token"


def test_status_error_truncates_the_body_to_200_chars():
    exc = _status_error(502, "x" * 500)
    err = translate_httpx_error("http_tts", exc)
    assert str(err) == f"http_tts returned HTTP 502: {'x' * 200}"
...
@pytest.mark.parametrize("name", ["http_stt", "http_tts", "remote_whisper", "openrouter"])
