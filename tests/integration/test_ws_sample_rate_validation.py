...
("a non-positive value has no honest duration to bill"); conversation.py and
livehost.py did the same cast with no guard, so `?sample_rate=0` reached
VadEndpointer and divided by it, and `?sample_rate=abc` raised ValueError --
both AFTER websocket.accept(), i.e. as an unhandled crash in the handler
...
@pytest.mark.parametrize("value", BAD_RATES)
def test_livehost_stream_refuses_a_bad_sample_rate(client, value):
    with client.websocket_connect(
        f"/v1/livehost/stream?sample_rate={value}"
    ) as ws:
        message = ws.receive_json()
    assert message["event"] == "error"
    assert "sample_rate" in message["message"]
