...
import { conv, convStopAudio, updateConvEnginesInfo, annotateLabel } from "./voice-stream.js";
import { STREAM_SAMPLE_RATE, createMicCapture } from "./audio-capture.js";

export const CONVERSATION_MODES = {
  "text-text":   { title: "Text Conversation",      hint: "Text conversation with the LLM." },
  "voice-voice": { title: "Voice Conversation",     hint: "Speak — VAD detects your pause, STT transcribes, the LLM replies, TTS speaks the reply." },
...
export let conversationMode = "text-text";
export const v2t = { ws: null, capture: null, assistantBubble: null };

export const conversationState = { history: [], busy: false };
...
export async function sendConversationMessage() {
  const input = el("conversation-input");
  const text = input.value.trim();
  if (!text || conversationState.busy) return;
  conversationState.busy = true;
  el("conversation-send").disabled = true;
  input.value = "";

  conversationState.history.push({ role: "user", content: text });
  conversationBubble("user", text);
  const pending = conversationBubble("assistant", "…");
  ...
  try {
    ...
    const conversationUrl = qs ? `/v1/conversation/chat?${qs}` : "/v1/conversation/chat";
    // Send only the new turn: the backend prefixes stored session context by
    // session_id, so resending conversationState.history here would double up
    // persisted messages and duplicate context sent to the LLM on every
    // subsequent turn.
    const resp = await fetch(conversationUrl, {
      method: "POST",
    ...
    if (!resp.ok) {
      ...
      conversationState.history.push({ role: "assistant", content: body.data.reply });
      if (body.data.session_id) currentSessionId = body.data.session_id;
      el("conversation-hint").textContent = `Responder: ${body.data.responder}${body.data.responder === "llm" ? " · " + body.data.model : " (no LLM configured — enable one in Model Registry)"}`;
    }
  } catch (error) {
    ...
    el("conversation-send").disabled = false;
    input.focus();
  }
}

el("conversation-send").addEventListener("click", sendConversationMessage);
el("conversation-input").addEventListener("keydown", (e) => {
  if (e.key === "Enter" && !e.shiftKey) {
    e.preventDefault();
    sendConversationMessage();
  }
});
el("conversation-reset").addEventListener("click", () => {
...
export function setConversationMode(mode) {
  ...
  document.querySelectorAll("#conversation-mode-seg .seg-btn").forEach((b) => {
  ...
  const info = CONVERSATION_MODES[mode] || {};
  if (el("conversation-section-title")) el("conversation-section-title").textContent = info.title || "Conversation";
  if (el("conversation-hint")) el("conversation-hint").textContent = info.hint || "";
  updateConvEnginesInfo(mode);
}

export function initConversationModes() {
  document.querySelectorAll("#conversation-mode-seg .seg-btn").forEach((btn) => {
    btn.addEventListener("click", () => setConversationMode(btn.getAttribute("data-mode")));
  });
  setConversationMode("text-text");
}
...
// session_id shared across all conversation modes instead of Voice→Text being
// a bare STT passthrough with no LLM turn.
  ...
  ws.onmessage = (event) => {
    ...
    switch (d.event) {
      case "session_started": {
        ...
        // (mirrors Voice→Voice's session_started handling in voice-stream.js).
      ...
      case "user_transcript":
        if (d.text) conversationBubble("user", d.text);
        break;
      case "response_text":
        if (d.chunk_index === 0 || !v2t.assistantBubble) {
          v2t.assistantBubble = conversationBubble("assistant", d.text);
        } else {
...
// ============================================================ text→voice (in conversation section)
// A conversation turn with a spoken answer (NOT raw text-to-speech): type text
// -> LLM replies -> the reply is synthesized and played. History/session/
// transcript are handled IDENTICALLY to Text→Text -- same
// conversationState.history, same shared #conversation-dialogue bubbles, same
// currentSessionId (so New session / Sessions / Reset and multi-turn context
// all work the same across modes). It just adds a spoken reply on top via the
// server's default TTS engine. Pure STT/TTS utilities live in Voice→Text.
async function sendTextToVoice() {
  ...
  // Same transcript + history as sendConversationMessage().
  conversationState.history.push({ role: "user", content: text });
  conversationBubble("user", text);
  const pending = conversationBubble("assistant", "…");

  try {
    // 1) text -> LLM (same endpoint/params as sendConversationMessage: profile + session apply).
    const profileVal = el("profile-select")?.value;
    const params = new URLSearchParams();
    if (profileVal) params.set("profile", profileVal);
    if (currentSessionId) params.set("session_id", currentSessionId);
    const qs = params.toString();
    const conversationResp = await fetch(qs ? `/v1/conversation/chat?${qs}` : "/v1/conversation/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages: [{ role: "user", content: text }] }),
    });
    const conversationBody = await conversationResp.json();
    if (!conversationResp.ok) {
      pending.textContent = `error: ${conversationBody.error || JSON.stringify(conversationBody)}`;
      pending.classList.add("error");
      return;
    }
    const reply = (conversationBody.data?.reply || "").trim();
    pending.textContent = reply || "(no reply)";
    conversationState.history.push({ role: "assistant", content: reply });
    if (conversationBody.data?.session_id) currentSessionId = conversationBody.data.session_id;
    if (!reply) return;
